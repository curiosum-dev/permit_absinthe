...
- Igniter installer task (`mix permit_absinthe.install`) to automatically patch the Absinthe schema module with `use Permit.Absinthe` (#14)

## [0.2.1] - 2026-02-24

### Changed

- Shortened middleware module name: `Permit.Absinthe.Middleware.LoadAndAuthorize` is now `Permit.Absinthe.Middleware` (#11). The old module is deprecated and delegates to the new one with a runtime warning.

### Fixed

- Fixed create action authorization: authorization is now checked against a blank struct for `:create` actions (and any custom actions that group under `:create`), preventing conditioned rules from passing unconditionally (#10).
...
[0.2.1]: https://github.com/permit-elixir/permit_absinthe/releases/tag/v0.2.1
